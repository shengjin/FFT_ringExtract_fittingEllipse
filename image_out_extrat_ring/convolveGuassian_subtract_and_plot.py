...
image_dust_conv = image_dust.imConv(fwhm, gaus_pa_ctclk, dpc)
...
if remove_dust:
    ...
    for i in range(image.nwav):
        image_conv.image[:,:,i] = image_conv.image[:,:,i] - image_dust_conv.image[:,:,0]
        image_conv.imageJyppix[:,:,i] = image_conv.imageJyppix[:,:,i] - image_dust_conv.imageJyppix[:,:,0]
...
#
for i in range(image.nwav):
    image_ave.image[:,:,0] = image_ave.image[:,:,0] + image_conv.image[:,:,i]*dv
    image_ave.imageJyppix[:,:,0] = image_ave.imageJyppix[:,:,0] + image_conv.imageJyppix[:,:,i]*dv
...
ImagePlot = image_ave.imageJyppix[:,:,n_image]


sizepix = image_ave.sizepix_x
print "image_ave.sizepix_x/au,image.sizepix_x/au"
print image_ave.sizepix_x/au,image.sizepix_x/au
print nx, image_ave.nx
...
#plt.ylim(-180,180)
#plt.xlim(-180,180)
plt.pcolormesh(im_x_au, im_y_au, ImagePlot*1000, vmin=0, vmax=50)
