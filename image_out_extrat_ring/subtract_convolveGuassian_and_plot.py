...
plt.pcolormesh(im_x_au, im_y_au, image_ave.imageJyppix[:,:,0]/dpc/dpc )
plt.xlabel('AU')
plt.ylabel('AU')
#plt.ylim(-180,180)
#plt.xlim(-180,180)
...
for i in range(n_row):
    for j in range(n_col):
        if colorlog:
            im=axes[i,j].pcolormesh(im_x_au, im_y_au, image.imageJyppix[:,:,n_col*i+j+jump]/dpc/dpc*1000, cmap='jet', norm=LogNorm(linthresh=0.03, linscale=0.03, vmin=v_min, vmax=v_max))
        else:
            im=axes[i,j].pcolormesh(im_x_au, im_y_au, image.imageJyppix[:,:,n_col*i+j+jump]/dpc/dpc*1000, cmap='jet', vmin=v_min, vmax=v_max)
